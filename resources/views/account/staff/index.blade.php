...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <a class="btn btn-success mb-2" href="{{ route('staff.create') }}">Create</a>
            <div class="form-group">
                <select name="" id="select-name"></select>
            </div>
            <table class="table table-hover table-centered" class="display custom-datatable" id="table-index"
                   style="width: 100%">
                <thead>
                <tr>
                    <th>#</th>
                        ...
                        @if(session()->get('role_id')===1)
                    {
                        ...
                        render: function (data, type, row, meta) {
                            return `<form action="${data}" method="post">
                                @csrf
                            @method('DELETE')
                            <button class="btn-delete btn btn-danger" class="btn_delete" type="button" style="font-size:20px"><i class=" uil-trash-alt "></i ></button>
                        </form>`;
                        }
                    },
                    @endif
                ...
                confirmDeleteBtn.click(function () {
                    $.ajax({
                        ...
                        success: function () {
                            console.log("success");
                            table.draw();
                            $('.error').text("Delete successfully");
                            $('.error').show();
                            $('.error').css({
